...
function clearAll() {
  const listOfDivs = ['.memes', '.joke', '.quote', '.riddle'];
  listOfDivs.forEach((item) => {
    const tempDiv =  document.querySelector(item);
...
function showMeme() {
  ...
  clearAll();
  const divMemes = document.querySelector('.memes');
  const randomMemeUrl = getRandomData('memes');
  const tempMeme = document.createElement('img');
  //tempMeme.classList.add("img");
  tempMeme.src = randomMemeUrl;
  divMemes.appendChild(tempMeme);  
}
...
function showJoke() {
  ...
  clearAll()
  const divJokes = document.querySelector('.joke');
  const randomJokeText = getRandomData('jokes');
  const tempJoke = document.createElement('p');
  //tempJoke.classList.add("jokes");
  tempJoke.textContent = randomJokeText;
  divJokes.appendChild(tempJoke);
}
...
function showQuote() {
  ...
  //tempQuote.classList.add("quote");
  tempQuote.textContent = (randomQuote.quote + ' -By '+  randomQuote.author);
  divQuote.appendChild(tempQuote);
}
...
function showRiddle() {
  ...
  const tempRiddle = document.createElement('p');
  tempRiddle.classList.add('rid');
  tempRiddle.textContent = randomRiddle.question;
  divRiddle.appendChild(tempRiddle);
...
function revealAnswers() {
  ...
  const divRiddle = document.querySelector('.riddle');
  const tempAns = document.createElement('p');
  tempAns.classList.add('rid');
  
  if (document.getElementsByClassName('rid').length === 0){
    alert('THERE IS NO RIDDLE')
  } else if(document.getElementsByClassName('rid').length === 2){ 
    alert('ANSWER IS ALREADY REVEALED')} else {
      for (i = 0; i < data['riddles'].length; i++){
        if(data['riddles'][i].question === document.getElementsByClassName('rid')[0].textContent){
          tempAns.textContent = data['riddles'][i].answer;
          divRiddle.appendChild(tempAns);}
      };
    };
}
